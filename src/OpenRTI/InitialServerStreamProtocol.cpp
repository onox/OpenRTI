...
InitialServerStreamProtocol::readOptionMap(const StringStringListMap& clientOptionMap)
{
  ...
  // Currently only version "3beta" is supported on both sides, currently just something to be
  ...
  if (!contains(i->second, "3beta")) {
    errorResponse("Client does not support version 3beta of the protocol.");
    return;
  }
  ...
  responseValueMap["version"].push_back("3beta");
